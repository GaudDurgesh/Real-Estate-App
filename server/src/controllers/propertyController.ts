...
export const getProperties = async (
  ...
  try {
    ...
    if (favoriteIds) {
      const favoriteIdsArrays = (favoriteIds as string).split(",").map(Number);
      whereConditions.push(
        Prisma.sql`p.id IN (${Prisma.join(favoriteIdsArrays)})`,
      );
    }
...
export const createProperty = async (
  ...
  try {
    ...
    if (!result?.lon || !result?.lat) {
      res.status(400).json({
        message: "Address location not found. Please check the address and postal code.",
      });
      return;
    }
